...
void create_index_init(CreateIndex *create_index, const char *index_name, 
                       const char *relation_name, Value values[], size_t value_num, const int is_unique_index) {
  create_index->index_name = strdup(index_name);
  create_index->relation_name = strdup(relation_name);
  for (size_t i = 0; i < value_num; i++) {
    create_index->attribute_name[i] = strdup((char*)values[i].data);
    LOG_INFO("i = %d, attribute_name = %s", i, create_index->attribute_name[i]);
  }
  create_index->attribute_count = value_num;
  create_index->is_unique_index = is_unique_index;
}
//20211107:e
//20211103:e
...
void drop_index_init(DropIndex *drop_index, const char *index_name, const char *relation_name, const char *attr_name) {
  ...
  drop_index->attribute_name = strdup(attr_name);
}
//20211105:e
